...
function __decorate(decorators, target, key2, desc) {
  ...
  else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c2 < 3 ? d(r) : c2 > 3 ? d(target, key2, r) : d(target, key2)) || r;
...
// packages/plugin/src/credential.mjs
import { connect } from "node:net";
import { readFileSync as readFileSync2, writeFileSync as writeFileSync2, mkdirSync as mkdirSync3, chmodSync as chmodSync2 } from "node:fs";
  ...
  */
  ...
  then(onfulfilled, onrejected) {
    ...
    const executeWithRetry = async () => {
      ...
      while (true) {
        const headers = {};
        _this.headers.forEach((value, key2) => {
          headers[key2] = value;
        });
...
var PostgrestTransformBuilder = class extends PostgrestBuilder {
  ...
  */
  limit(rows, { foreignTable, referencedTable = foreignTable } = {}) {
    ...
    this.url.searchParams.set(key2, `${rows}`);
    return this;
  }
  /**
  ...
  */
  async remove(paths2) {
    ...
    return _this12.handleOperation(async () => {
      return await remove(_this12.fetch, `${_this12.url}/object/${_this12.bucketId}`, { prefixes: paths2 }, { headers: _this12.headers });
    });
  }
  /**
...
var isNewApiKey = (key2) => key2.startsWith("sb_publishable_") || key2.startsWith("sb_secret_");
var TEMP_KEY_PREFIX = "sb_temp_";
var warnedKeySubtypes = /* @__PURE__ */ new Set();
var checkApiKeyFormat = (key2) => {
  var _key$match$, _key$match;
  if (!key2.startsWith("sb_") || isNewApiKey(key2) || key2.startsWith(TEMP_KEY_PREFIX)) return;
  const subtype = (_key$match$ = (_key$match = key2.match(/^sb_[a-zA-Z0-9]+_/)) === null || _key$match === void 0 ? void 0 : _key$match[0]) !== null && _key$match$ !== void 0 ? _key$match$ : "unknown";
...
// packages/plugin/src/identity.mjs
import { mkdirSync as mkdirSync2, readFileSync, writeFileSync, existsSync, chmodSync } from "node:fs";
import { homedir as homedir2 } from "node:os";
import { join as join2, dirname } from "node:path";
var DIR = process.env.FLOOR_HOME ?? join2(homedir2(), ".floor");
var SESSION = join2(DIR, "session.json");
var ROOM = join2(DIR, "room.json");
var SOCK = sockPath();
var URL2 = process.env.FLOOR_SUPABASE_URL ?? "https://rjbdbsqbjoygadbwimhc.supabase.co";
var ANON_KEY = process.env.FLOOR_SUPABASE_ANON_KEY ?? "sb_publishable_w9FqOv2KKDDu_PwVZeb4yQ_elZb41EB";
function read() {
  try {
...
var PRIVATE = { mode: 384 };
function writePrivate(path, contents) {
  writeFileSync(path, contents, PRIVATE);
  try {
    chmodSync(path, 384);
  } catch {
  }
}
function write(session2) {
  mkdirSync2(dirname(SESSION), { recursive: true, mode: 448 });
  writePrivate(SESSION, JSON.stringify(session2, null, 2));
}
async function device() {
  if (!ANON_KEY) {
    throw new Error(
      "FLOOR_SUPABASE_ANON_KEY is not set. The plugin needs it to reach Floor \u2014 it is the publishable key that ships in every page, not a secret."
    );
  }
  const client = createClient(URL2, ANON_KEY, {
    auth: {
      persistSession: false,
      autoRefreshToken: false,
      /**
       * Pinned, not left to the library default, because `floor_link` depends
       * on which one it is. PKCE keeps a code verifier in this process's
       * storage and hands the browser a `?code=` that only this process could
       * exchange — which is no use, because the browser is the one that needs
       * the session. Implicit puts the whole flow in the URL: GoTrue links the
       * identity at its own callback and redirects with a fresh session in the
       * fragment, so the browser signs itself in and this process never has to
       * hand over the session it is already using. Measured, not assumed —
       * under pkce the verifier lands in three local keys and under implicit
       * in none.
       */
      flowType: "implicit"
    }
  ...
  if (error) {
    throw new Error(`Floor could not register this machine: ${error.message}`);
  }
  write(data.session);
  return {
    client,
...
var paths = { DIR, SESSION, ROOM, SOCK };
...
// packages/plugin/src/site.mjs
var DEFAULT_SITE = "https://multiplayerai-tower-web.vercel.app";
function siteOrigin() {
  return (process.env.FLOOR_SITE_URL ?? DEFAULT_SITE).replace(/\/+$/, "");
}
...
async function requestToken(accessToken2, repo2) {
...
function repoFromCredentialRequest(req2) {
  if (req2?.protocol !== "https" || req2?.host !== "github.com" || !req2?.path) return null;
  const m = req2.path.replace(/^\/+/, "").replace(/\.git$/, "").match(/^([^/]+\/[^/]+)$/);
  return m ? m[1] : null;
}

// packages/plugin/src/credential.mjs
var op = process.argv[2];
var CACHE = join4(paths.DIR, "git-tokens.json");
var req = {};
try {
  for (const line of readFileSync2(0, "utf8").split("\n")) {
    const i = line.indexOf("=");
    if (i > 0) req[line.slice(0, i)] = line.slice(i + 1);
  }
} catch {
}
var repo = repoFromCredentialRequest(req);
if (!repo || op !== "get" && op !== "erase") process.exit(0);
function readCache() {
  try {
...
function writeCache(c2) {
  try {
    ...
    writeFileSync2(CACHE, JSON.stringify(c2), { mode: 384 });
    chmodSync2(CACHE, 384);
  } catch {
  }
}
function askServer(message) {
  return new Promise((resolve) => {
    if (ensurePrivateSockDir()) return resolve(null);
    let done = false;
    const finish = (v) => {
      if (!done) {
        done = true;
...
if (op === "erase") {
  ...
  delete c2[key];
  writeCache(c2);
  await askServer({ floor: "git-credential", op: "erase", repo });
  process.exit(0);
}
var say = (token) => {
...
var fromServer = await askServer({ floor: "git-credential", op: "get", repo });
if (fromServer?.ok === true && fromServer.token) say(fromServer.token);
if (fromServer && fromServer.ok === false) {
  process.stderr.write(
    `floor: no access to ${repo} through Floor \u2014 ${fromServer.message ?? fromServer.reason ?? "refused"}.
`
  );
  process.exit(0);
}
var cached = readCache()[key];
if (cached?.token && Date.parse(cached.expiresAt) - Date.now() > 5 * 6e4) say(cached.token);
var session = null;
try {
  session = JSON.parse(readFileSync2(paths.SESSION, "utf8"));
} catch {
}
if (!session?.access_token) {
  process.stderr.write(
    "floor: this machine is not registered with Floor yet \u2014 open Claude Code and /floor:join a room first.\n"
  );
  process.exit(0);
}
var accessToken = session.access_token;
if ((session.expires_at ?? 0) * 1e3 - Date.now() < 6e4) {
  const { client } = await device();
  const { data } = await client.auth.getSession();
  accessToken = data?.session?.access_token ?? null;
}
if (!accessToken) process.exit(0);
var minted = await requestToken(accessToken, repo);
if (minted.ok !== true) {
  process.stderr.write(
    `floor: no access to ${repo} through Floor \u2014 ${minted.message ?? minted.reason}.
`
  );
  process.exit(0);
}
var c = readCache();
c[key] = { token: minted.token, expiresAt: minted.expiresAt };
writeCache(c);
say(minted.token);
